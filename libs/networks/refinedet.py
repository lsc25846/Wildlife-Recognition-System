...
from libs.utils.net_utils import TCB,TCB_a, make_special_tcb_layer, weights_init
...
class RefineDet(nn.Module):
    ...
    def _init_part_modules(self):
        ...
        self.pyramid_layer2 = TCB_a(self.layers_out_channels[1], self.internal_channels,
                             self.internal_channels, self.is_batchnorm)
        self.pyramid_layer3 = TCB_a(self.layers_out_channels[2], self.internal_channels,
                             self.internal_channels, self.is_batchnorm)
    ...
    def forward(self, x, targets=None):
        ...
        f1=f1.astype(int)
        #for i in range (512):
        #    c1_nparr[0][i]=c1_nparr[0][i]*256
        #    np.savetxt('c'+str(i)+'file.txt',c1_nparr[0][i],fmt='%d')
        ...
        p1_nparr = p4.cpu().detach().numpy()
        #for i in range (256):
        #    p1_nparr[0][i]=p1_nparr[0][i]*256
        #    np.savetxt('p'+str(i)+'file.txt',p1_nparr[0][i],fmt='%d') 
