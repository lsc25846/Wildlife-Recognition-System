...
class Detect(nn.Module):
    ...
    def forward(self, odm_predictions, refined_anchors,
        ...
        output = torch.zeros(num, self.num_classes, self.top_k_per_class,
                             5).type_as(loc_data)
        output2 = torch.zeros(num, self.num_classes, self.top_k_per_class,
                             5).type_as(loc_data)
        # select
        ...
        for idx in range(num):
            ...
            #print(output[0][11])
            a_scores=torch.zeros((output.size(1)*output.size(2)))
            a_boxes=torch.zeros((output.size(1)*output.size(2)),4)
            temp=torch.zeros(output.size(2),output.size(3))
            c=0
            for i in range(output.size(1)):
                for j in range (output.size(2)):
                    for k in range (output.size(3)):
                        if k ==0:
                            a_scores[c]=output[0][i][j][k]
                        else:
                            a_boxes[c][k-1]=output[0][i][j][k]
                    c+=1


            a_ids, a_count = nms(a_boxes, a_scores, self.nms_thresh,
                                 self.top_k_per_class)
            #print(a_ids,a_count)
            t_c_class = -1
            count_per_class = 1
            ...
            for l in range (a_count):
                ...
                if t_c_class == cl:
                    ...
                    #print('cl&t_c_class=',cl,t_c_class)
                    if t_c_class != -1:
                        class_end=count_per_class+class_head
                        #print(class_head,class_end)
                        output2[idx, t_c_class, class_head : class_end]=\
                            torch.cat((a_scores[a_ids[class_head : class_end]].unsqueeze(1),
                                       a_boxes[a_ids[class_head : class_end]]), 1)
                        class_head=class_end
                    t_c_class=cl
                    count_per_class=1
            class_end=count_per_class+class_head
            output2[idx, t_c_class, class_head : class_end]=\
        ...
        # pdb.set_trace()
        flt = output2.contiguous().view(num, -1, 5)
